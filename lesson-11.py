...
# Переобученные модели очень чувствительны к выбросам, которые находятся далеко от остальных точек, в прогнозах будет высокая дисперсия.
# Поэтому к моделям специально добавляется смещение.
# Смещение модели означает, что предпочтение отдаётся определённой схеме (например, прямая линия, а не график со сложной структурой, минимизирующий
# сумму остатков).
# Если в модель добавить смещение, то есть риск недообучения модели. Необходима балансировка между минимизацией функции потерь (переобучение) 
# и смещением (недообучение).
# - Гребневая регрессия (ridge): добавляется смещение в виде штрафа, из-за этого хуже идёт подгонка.
# - Лассо-регрессия: удаление некоторых переменных.
# Механически применить линейную регрессию к данным, сделать на основе полученной модели прогноз и думать, что всё в порядке, нельзя.
...
kfold = KFold(n_splits=3, random_state=1, shuffle=True) # 3-х кратная перекрёстная валидация
...
# Коэффициент детерминации (чем ближе к 1, тем лучше регрессия работает на тестовых данных)
...
# Случайная валидация позволяет перемешивать данные и разделять их на две выборки многократно
